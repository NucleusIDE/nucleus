Files = (function() {
    ...
    var  mkDirTree= function () {
      ...
      walk(rootDir, function(node) {
        if(!! node && !! node.parentId) { //don't push falsy rows or the root node
          ...
          node.appPath = node.id.replace(rootDir, '').replace(node.name, ''); //we don't want to show absolute name
          tree.push(node);
        }
      });
  ...
  FileManager.prototype.getFileExtension = function (filepath) {
    return path.extname(filepath).replace(".", "");
  };

  FileManager.prototype.getFileContents = function(filepath) {
    /**
  ...
  FileManager.prototype.createNewFile = function(filepath, directory) {
    /**
  ...
  FileManager.prototype.saveDocToFile = function(docId) {
    /**
  ...
  FileManager.prototype.setupFileForEditting = function(filepath, forceRefresh) {
    /**
